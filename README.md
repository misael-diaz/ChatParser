...
- **Spatial Mappings**: Conducting the spatial mappings of the user and chat messages involved pointer arithmetic. The surest way to determine the length of the chat was by leveraging the offset to the next timestamp. This is important because the newline character alone is not an indication of the end of a message. The last message does not contain a following timestamp but we know how many bytes are in the text and so we used that to determine its length. The nice thing about this is that the spatial mappings were possible without introducing regular expressions&mdash;a tool that will prove its usefulness later.
...
- **Fixed Time Value Mapping**: Fixed the location of the day value in the timestamp generated by WhatsApp. This one was another of the cases caught by the defensive programming code.
